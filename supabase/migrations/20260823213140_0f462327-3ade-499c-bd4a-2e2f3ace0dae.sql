...
BEGIN
  ...
  IF NOT (
    ...
    OR (private.is_staff(v_supervisor_id) AND private.my_team(v_supervisor_id) = v_team)
  ) THEN
...
REVOKE ALL ON FUNCTION public.pay_fair_push_charge(uuid, integer, text, integer, text) FROM PUBLIC, anon;
