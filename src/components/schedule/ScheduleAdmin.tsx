...
import { Trash2, Wand2, Send, Loader2, CalendarDays, EyeOff } from 'lucide-react';
import { toast } from 'sonner';
import { supabase } from '@/integrations/supabase/client';
import { extractDate, parseScheduleText, type ParsedScheduleItem } from '@/lib/scheduleParser';
...
const ScheduleAdmin = () => {
  ...
  const [draft, setDraft] = useState<ParsedScheduleItem[] | null>(null);
  ...
  const parse = () => {
    ...
    if (!items.length) { toast.error('Не вдалось розпізнати жодної події'); return; }
    const d = extractDate(raw);
    if (d) setDate(d);
    setDraft(items);
    toast.success(`Розпізнано ${items.length} подій`);
  };

  const patch = (idx: number, values: Partial<ParsedScheduleItem>) =>
  ...
  return (
    <div className="space-y-4">
      <Card className="p-5 bg-gradient-card space-y-3">
        <h3 className="font-bold uppercase text-sm tracking-wide">Розклад з тексту</h3>
        <Textarea
          value={raw}
        ...
        />
        <Button onClick={parse} className="w-full h-11 font-bold uppercase">
          <Wand2 className="w-4 h-4 mr-2" /> Розібрати текст
        </Button>
      </Card>
      ...
      {draft && (
        <Card className="p-4 bg-gradient-card space-y-3">
          ...
          <div className="space-y-2 max-h-[60vh] overflow-y-auto scrollbar-thin">
            {draft.map((it, i) => (
              <div key={i} className="rounded-xl border border-border/50 bg-surface-1 p-3 space-y-2">
                ...
                <Input value={it.description ?? ''} onChange={(e) => patch(i, { description: e.target.value || null })} placeholder="Опис (необов'язково)" className="h-9 text-xs" />
                <div className="space-y-1">
                  <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Команди (через кому, порожньо = загальне)</Label>
                  <Input
                    value={it.target_teams.join(', ')}
                    onChange={(e) => patch(i, {
                      target_teams: e.target.value.split(',').map((s) => parseInt(s.trim(), 10)).filter((n) => Number.isFinite(n)),
                    })}
                    placeholder="1, 2"
