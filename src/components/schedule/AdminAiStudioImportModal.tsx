...
const AdminAiStudioImportModal = ({ open, date, onOpenChange, onImported }: Props) => {
  ...
  const copyPrompt = async () => {
    if (!rawText.trim()) { toast.error('Спочатку встав текст розкладу'); return; }
  ...
  const publish = async () => {
    ...
    try {
      parsed = JSON.parse(json.trim().replace(/^```json/i, '').replace(/```$/, '').trim());
    } catch {
      toast.error('Некоректний JSON. Скопіюй відповідь ШІ повністю, без зайвого тексту.');
      return;
    }
    const items = Array.isArray(parsed?.items) ? parsed.items : [];
    if (!items.length) { toast.error('У JSON немає жодної події (items)'); return; }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[88vh] overflow-y-auto">
        <DialogHeader><DialogTitle>Імпорт розкладу через AI Studio</DialogTitle></DialogHeader>

        <div className="space-y-4">
          <div className="space-y-1.5">
            <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Крок 1 — сирий текст розкладу</Label>
            <Textarea
              value={rawText}
          ...
          <div className="space-y-2">
            <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Крок 2 — згенеруй JSON у ШІ</Label>
          ...
          <div className="space-y-1.5">
            <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Крок 3 — вставити готовий JSON з AI Studio</Label>
            <Textarea
              value={json}
          ...
          <Button onClick={publish} disabled={busy || !json.trim()} className="w-full h-12 text-xs font-bold uppercase">
            {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : (<><CheckCircle2 className="w-4 h-4 mr-1.5" /> Завантажити та опублікувати розклад</>)}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
