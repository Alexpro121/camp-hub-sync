    ...
    const restoreUserSession = async () => {
      try {
        ...
        if (savedRole === 'child') {
          ...
          // Якщо паспорт дитини порожній — скидаємо
          clearSavedSession();
          if (!cancelled) {
            setScreen('role');
            setRestoring(false);
        ...
        if (savedRole === 'supervisor' || savedRole === 'admin') {
          ...
          // Якщо є активна сесія Supabase Auth — пускаємо у відповідний кабінет
          if (!error && data?.session) {
            setScreen(savedRole);
          } else {
            // Якщо для супроводу збережено локальний ключ команди
